...
function validateRefundOfferResponse(refundOffer, index, expectedRefundOperationStatus, expectedFulfillmentStatus) {
  ...
  test(`Refund offer[${index}] refundableAmount exists and is valid, amount: ${refundOffer.refundableAmount.amount}, currency: ${refundOffer.refundableAmount.currency}`, () => {
  ...
  test(`Refund offer[${index}] refundFee exists and is valid, amount: ${refundOffer.refundFee.amount}, currency: ${refundOffer.refundFee.currency}`, () => {
  ...
  if (Array.isArray(refundOffer.refundOfferBreakDown) && refundOffer.refundOfferBreakDown.length > 0) {
    ...
    refundOffer.refundOfferBreakDown.forEach((breakdown, bdIndex) => {
      test(`Refund offer[${index}] breakdown[${bdIndex}] is valid, refundFee amount: ${breakdown.refundFee.amount}, refundableAmount amount: ${breakdown.refundableAmount.amount}`, () => {
        // Validate refundFee
...
function getBookingRefundResponse(response, scenarioType) {
  ...
  if (["postRefund", "patchRefund"].includes(scenarioType)) {
    const idsAdmissionAncillariesReservationReference = JSON.parse(bru.getEnvVar("idsAdmissionAncillariesReservationReferenceDummy") || "[]");

...
function ensureAuthorizationOr403() {
  ...
  try {
    ...
    bru.sendRequest({
      ...
      body: typeof resolvedBody === 'string' ? resolvedBody : (resolvedBody ? JSON.stringify(resolvedBody) : undefined),
      proxy: false
    }, function (err, res) {
      if (err) {
      ...
      if (code === 403 || code === 401) {
        ...
        throw new Error("Authorization precheck failed with " + code);
      } else if (code === 400) {
        console.log("⛔ Stop: Bad Request (400). Check request parameters and body. Possibly due to authorization.");
        throw new Error("Authorization precheck failed with 400");
      } else {
...
function validateAdmissions(selectedOffer) {
  ...
  const reservationParts = selectedOffer.reservationOfferParts || [];
  const ancillaryParts = selectedOffer.ancillaryOfferParts || [];
  const admissionReservationAncillaryOfferPartsIds = bru.getEnvVar("admissionReservationAncillaryOfferPartsIds") || [];
  ...
  if (admissionParts.length > 0) {
    ...
    validationLogger(`[INFO] No admissionOfferParts found for offer.id : ${selectedOffer.id} → test skipped`);
  }
}
...
function validateReservations(selectedOffer) {
  ...
  if (reservationParts.length > 0) {
    ...
    validationLogger(`[INFO] No reservationOfferParts found for offer.id : ${selectedOffer.id} → test skipped`);
  }
}
    ...
    admissionParts.forEach((admission, i) => {
      ...
      if (scenarioType.includes("REFUND")) {
        if (admission.refundable !== "YES") {
          ...
          throw new Error("Admission part not refundable in REFUND scenario");
        } else {
        ...
        if (admission.exchangeable !== "YES") {
          ...
          throw new Error("Admission part not exchangeable in EXCHANGE scenario");
        } else {

...
function displayBookingResponse(response) {
  ...
  validationLogger(
    `[FULL] Provisional Price: ${response.booking.provisionalPrice.amount} ${response.booking.provisionalPrice.currency}`
  );
  ...
  response.booking.passengers.forEach((passenger, passengerIndex) => {
    ...
    validationLogger(
      `[FULL]   Cards: ${passenger.cards ? passenger.cards.join(", ") : "None"}`
    );
  });
  ...
  response.booking.trips.forEach((trip, tripIndex) => {
    ...
    trip.legs.forEach((leg, legIndex) => {
      ...
      validationLogger(
        `[FULL]       Start Time: ${leg.timedLeg.start.serviceDeparture.timetabledTime}`
      );
      validationLogger(
        `[FULL]       End Time: ${leg.timedLeg.end.serviceArrival.timetabledTime}`
      );
      ...
      validationLogger(
        `[FULL]       Line Numbers: ${
          leg.timedLeg.service.lineNumbers ? leg.timedLeg.service.lineNumbers.join(", ") : "None"
        }`
      );
    });
  });
  ...
  response.booking.bookedOffers.forEach((offer, offerIndex) => {
    ...
    offer.reservations.forEach((reservation, reservationIndex) => {
      ...
      validationLogger(
        `[FULL]       Price: ${reservation.price.amount} ${reservation.price.currency}`
      );
